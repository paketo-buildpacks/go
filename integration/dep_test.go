...
func testDep(t *testing.T, context spec.G, it spec.S) {
	...
	context("when building a go app using dep", func() {
		...
		it("creates a working OCI image", func() {
			...
			Expect(filepath.Join(sbomDir, "sbom", "build", "paketo-buildpacks_dep", "dep", "sbom.cdx.json")).To(BeARegularFile())
			Expect(filepath.Join(sbomDir, "sbom", "build", "paketo-buildpacks_dep", "dep", "sbom.spdx.json")).To(BeARegularFile())
			Expect(filepath.Join(sbomDir, "sbom", "build", "paketo-buildpacks_dep", "dep", "sbom.syft.json")).To(BeARegularFile())
